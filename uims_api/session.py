...
class SessionUIMS:
    ...
    def _extract_timetable(self, response):
        ...
        start_colon = report_div_block + \
            response.text[report_div_block:].find(':')
        end_quotation = start_colon+2 + response.text[start_colon+2:].find('"')
        report_div_id = response.text[start_colon+2:end_quotation]
        soup = BeautifulSoup(response.text, 'html.parser')
        nearest_div_id = report_div_id[:report_div_id.find(
            'oReportDiv')] + '5iS0xB_gr'
    ...
    def _parse_timetable_subject(self, subject, course_codes):
        ...
        return_subject['teacher'] = teacher_name if pattern.match(
            teacher_name) else None
